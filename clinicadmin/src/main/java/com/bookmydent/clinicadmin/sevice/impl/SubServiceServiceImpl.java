...
import com.bookmydent.clinicadmin.dto.SubServicesDto;
import com.bookmydent.clinicadmin.exceptions.FeignClientException;
import com.bookmydent.clinicadmin.feignclient.ServiceFeignClient;
import com.bookmydent.clinicadmin.service.SubServiceService;
...
public class SubServiceServiceImpl implements SubServiceService {
	...
	public ResponseEntity<ResponseStructure<SubServicesDto>> addService(String subServiceId, SubServicesDto dto) {
		try {
			ResponseEntity<ResponseStructure<SubServicesDto>> response = feignClient.addService(token.access_token, subServiceId, dto);
	...
	public ResponseEntity<ResponseStructure<SubServicesDto>> deleteSubService(String hospitalId, String subServiceId) {
		try {
			ResponseEntity<ResponseStructure<SubServicesDto>> response = feignClient.deleteSubService(token.access_token,hospitalId,
					subServiceId);
			return ResponseEntity.status(response.getBody().getStatusCode()).body(response.getBody());}
	...
	public ResponseEntity<ResponseStructure<SubServicesDto>> updateBySubServiceId(String hospitalId, String serviceId,
		...
		try {
			ResponseEntity<ResponseStructure<SubServicesDto>> response = feignClient.updateBySubServiceId(token.access_token,hospitalId,
	...
	public ResponseEntity<ResponseStructure<List<SubServicesDto>>> getSubServiceByHospitalId(String hospitalId) {
	    try {
	        ResponseEntity<ResponseStructure<List<SubServicesDto>>> response =
	                feignClient.getSubServiceByHospitalId(token.access_token,hospitalId); // ✅ FIXED here
	...
	public ResponseEntity<ResponseStructure<List<SubServicesDto>>> getAllSubServices() {
		try {
			ResponseEntity<ResponseStructure<List<SubServicesDto>>> response = feignClient.getAllSubServices(token.access_token);
			return ResponseEntity.status(response.getBody().getStatusCode()).body(response.getBody());
