...
import com.bookmydent.clinicadmin.dto.BookingResponseDTO;
...
import com.bookmydent.clinicadmin.feignclient.BookingFeign;
import com.bookmydent.clinicadmin.repository.DoctorsRepository;
import com.bookmydent.clinicadmin.repository.PatientConsentFormRepository;
import com.bookmydent.clinicadmin.service.PatientConsentFormService;
...
public class PatientConsentFormServiceImpl implements PatientConsentFormService {
	...
	public Response getPatientDetailsForFormUsingBooking(String bookingId, String patientId, String mobileNumber) {
		...
		ResponseEntity<Response> responseEntity = bookingFeign.getPatientDetailsForConsentForm(token.access_token,bookingId, patientId,
				mobileNumber);
 ...
 ResponseEntity<Response> clinicData =	adminServiceClient.getClinicById(token.access_token,bookingDto.getClinicId());
 ClinicDTO clinics = objectMapper.convertValue(clinicData.getBody().getData(), ClinicDTO.class);
	if (clinics == null) {
		...
		ResponseEntity<Response> adminRes = adminServiceClient.getClinicById(token.access_token,bookingDto.getClinicId());
