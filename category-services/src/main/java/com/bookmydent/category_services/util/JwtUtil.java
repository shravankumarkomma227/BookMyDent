...
import com.bookmydent.category_services.dto.JwkKey;
import com.bookmydent.category_services.dto.JwksResponse;
import com.bookmydent.category_services.dto.Roles;
import com.bookmydent.category_services.feign.KeyCloakFeign;
...
public class JwtUtil {
	...
	public List<String> extractRoleFromToken(String token) {
		...
		try {
			...
			kId = new ObjectMapper().convertValue(clms.get("kid"),String.class);
			System.out.println(kId);
					Map<String, Object> claimsMap = new ObjectMapper().readValue(decodeTokenWithoutKey(token), Map.class);
					System.out.println(claimsMap);
					if(new ObjectMapper().convertValue(claimsMap.get("preferred_username"),String.class).equalsIgnoreCase("service-account-clinicadminservice@8080")){ 
						if(jWKSet == null){
							jWKSet = keyCloakFeign.getJWKS();}
