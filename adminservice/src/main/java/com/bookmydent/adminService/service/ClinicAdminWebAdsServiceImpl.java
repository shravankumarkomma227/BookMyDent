...
import com.bookmydent.adminService.dto.ClinicAdminWebAdsDTO;
import com.bookmydent.adminService.entity.ClinicAdminWebAds;
import com.bookmydent.adminService.repository.ClinicAdminWebAdsRepository;

import java.util.Base64;

@Service
public class ClinicAdminWebAdsServiceImpl {

    private final ClinicAdminWebAdsRepository adsRepository;
    ...
    public ClinicAdminWebAdsServiceImpl(ClinicAdminWebAdsRepository adsRepository) {
    ...
    public ClinicAdminWebAdsDTO createAd(ClinicAdminWebAdsDTO dto) {
        ClinicAdminWebAds ad = new ClinicAdminWebAds();
        ...
        ClinicAdminWebAds saved = adsRepository.save(ad);
        return new ClinicAdminWebAdsDTO(saved.getAdId().toHexString(), saved.getMediaUrlOrImage());
    }

    public Iterable<ClinicAdminWebAdsDTO> getAllAds() {
        List<ClinicAdminWebAdsDTO> list = new ArrayList<>();
        for (ClinicAdminWebAds ad : adsRepository.findAll()) {
            list.add(new ClinicAdminWebAdsDTO(ad.getAdId().toHexString(), ad.getMediaUrlOrImage()));
        }
        return list;
    }

    public Optional<ClinicAdminWebAdsDTO> getAdById(String adId) {
        return adsRepository.findById(new ObjectId(adId)).map(
                ad -> new ClinicAdminWebAdsDTO(ad.getAdId().toHexString(), ad.getMediaUrlOrImage())
        );
    }

    public Optional<ClinicAdminWebAdsDTO> updateAd(String adId, ClinicAdminWebAdsDTO dto) {
        return adsRepository.findById(new ObjectId(adId)).map(ad -> {
            ...
            ClinicAdminWebAds updated = adsRepository.save(ad);
            return new ClinicAdminWebAdsDTO(updated.getAdId().toHexString(), updated.getMediaUrlOrImage());
        });
    }
