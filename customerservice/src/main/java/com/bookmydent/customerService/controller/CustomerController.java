package com.bookmydent.customerService.controller;
...
import com.bookmydent.customerService.util.OtpUtil;
...
@GetMapping("/getRatingInfo/{hospitalId}/{branchId}/{doctorId}")
public ResponseEntity<Response> getRatingInfo(@PathVariable String hospitalId,@PathVariable String branchId ,@PathVariable String doctorId) {
	 Response response = customerService.getRatingForService( hospitalId,branchId,doctorId);
	 if(response != null && response.getStatus() != 0) {
		 return ResponseEntity.status(response.getStatus()).body(response);
	 }else {
			return null;
